...
type
 ...
 the_application = object(platform_support )
   ...
   procedure perform_rendering(
              ...
              clipper : conv_clipper_ptr );

   function  render_clipper(sl : scanline_ptr; ras : rasterizer_scanline_ptr ) : unsigned;
 ...
 if m_operation._cur_item > 0 then
  ...
  // Render clipped polygon
   ...
   case m_operation._cur_item of
    1: clipper.operation(clipper_or );
    2: clipper.operation(clipper_and );
    3: clipper.operation(clipper_xor );
    4: clipper.operation(clipper_a_minus_b );
    5: clipper.operation(clipper_b_minus_a );

   end;

   counter.Construct(clipper);

   start_timer;
   counter.rewind(0); /////////////////////////this is where it all happens!
   t1:=elapsed_time;
...
{ RENDER_CLIPPER }
function the_application.render_clipper;
var
 pf : pixel_formats;
 rb : renderer_base;
...
begin
 ...
 if key = key_f1 then
  message_(
   '''Clipper'' by Angus Johnson is the most reliable implementation of the '#13 +
   ...
   'several contours). AGG has a simple wrapper class that can be used in the '#13 +
   'coordinate conversion pipeline. Note that all operations are done '#13 +
   'in the vectorial representation of the contours before rendering.'#13#13 +
...
begin
 ...
 for i:=0 to 999 do
  begin
   ...
   case op of
    0 :
     clipper.operation(clipper_or );

    1 :
     clipper.operation(clipper_and );

    2 :
     clipper.operation(clipper_xor );

    3 :
     clipper.operation(clipper_a_minus_b );

    else
     clipper.operation(clipper_b_minus_a );

   end;

  // Clipping result
   ras.add_path    (@clipper );
   rgba.ConstrDbl  (0.5 ,0.0 ,0 ,0.5 );
